  ...
  class RRTstarPlannerROS : public nav_core::BaseGlobalPlanner 
  {
    ...
    public:
      ...
      */
      ...
      int MAX_NUM_NODES;      // The maximum number of new nodes that can be generated when planning a path
      float PROBABILITY_THRESHOLD; // The threshold for determining whether regard the goal point as the random point or sample a point randomly
      float RADIUS;          // The radius range of nearby nodes of new generated node during optimization
      float GOAL_TOLERANCE;  // Tolerance that allows new generated node to be considered as goal point
      float epsilon_min;     // The minimum value of the step length used when generating new node
      float epsilon_max;     // The maximum value of the step length used when generating new node
